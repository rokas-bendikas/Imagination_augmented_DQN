...
def main():
    ...
    parser.add_argument('--lr', default=5e-4, type=float, help='Learning rate for the training [default = 5e-6]')
    ...
    parser.add_argument('--warmup', default=10, type=int, help='How many full exploration iterations [default=10]')
    parser.add_argument('--accelerator', default=False, type=str2bool, help='Use model-based accelerator [default=True]')
    parser.add_argument('--model', default="A2C", type=str, help='What model to use [default="DQN"]')
...
def run_DQN(args):
    ...
    explorer = mp.Process(target=collect_DQN,args=(simulator,model_shared,queue,lock,args,flush_flag,warmup_flag,beta))
    optimiser = mp.Process(target=optimise_DQN,args=(simulator,model_shared,queue,lock,args,flush_flag,warmup_flag,beta))
